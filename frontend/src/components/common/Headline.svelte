<script>
    ...
    onMount(() => {
        newHeadline =
            lastAccessedAt < new Date(headline.publishedAt).getTime()
                ? true
                : false;
    });
...
<div
    class="rounded overflow-hidden ml-2 p-2"
    use:inview={headlineInViewOptions}
    on:inview_enter={handleHeadlineView}
>
    <div class="flex justify-between items-center">
        {#if sourceLogoInView}
            <img
                src="/img/sources/{source.name}.webp"
                class="inline-block rounded-lg pr-3"
        ...
        <div class="flex-1">
            ...
            >
                <a
                    ...
                    href={headline.url}
                    on:click|preventDefault={source.isScrapable
                        ? toggleStory
                        : null}
                >
                    {headline.title}
        ...
        {#if source.isScrapable}
            <button
                ...
                class="text-news-light focus:outline-none pl-1"
            >
                <svg
                    class="w-6 h-6"
                    fill="none"
    ...
    {#if showStory}
        <Story headlineUrl={headline.url} headlineId={headline.id} />
    {/if}
</div>
